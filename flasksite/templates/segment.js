...
function clickcontent(d) {
    ...
    $('.clickbox').replaceWith('<div class="clickbox">' + JSON.stringify(d) + '<br>tornadoes</div>');
...
function setMapHour(hour2digit) {
	...
	var tilejsonoverlay = {
	    ...
	    grids: ['http://{{ servername }}:{{ mapserverport }}/query3_o'+hour2digit+'/{z}/{x}/{y}.json?'],
	...
	//wax.leaf.interaction(map,tilejsonoverlay);
...
function updateURL(v) {
    ...
    var tinput = document.getElementById('wgtimeinput');	
    var dinput = document.getElementById('wgdateinput');	
    history.pushState('page', 'caption', '/static/segment.html?lat='+latlng.lat+'&lon='+latlng.lng+'&z='+map.getZoom()+'&m='+dopumode+'&e='+queryexecdone+'&date='+dinput.value+'&time='+tinput.value);
