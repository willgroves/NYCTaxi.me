'''
Segment Gradient Query -- for nyctaxi.me analysis area
...
regularize to 60 minute intervals on rows identified by
hour). Returns a dictionary of the mean arrival
interval along with some other information based on the query location.  
        ...
        if self.loadedid != identifi: ##compare loaded file's identifier with id of request
            ##do a load
        ...
        for i, result in enumerate(resultl):
            ...
            if result['m2m1ratio']>1.0: ##handle case where end 2 is better than end 1
            ...
            else: ##handle case end 1 is better than end 2
                result['heading_txt'] = "%.0f"%result['heading']
